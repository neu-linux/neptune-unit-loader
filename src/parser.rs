...
fn generate_unit_list(units: &[UnitFile]) -> Result<Vec<UnitFile>, GraphBuildError> {
    ...
    for unit in units {
        ...
        for dep in &unit.dependency.needs_before {
            if let Some(&to) = idx_map.get(dep) {
                ...
                return Err(GraphBuildError::LoadError(UnitLoadError::MissingDependency(unit.unit.unit_name.clone(), dep.clone())));
            }
        }
